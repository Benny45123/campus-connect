...
import AuthorHoverCard from './AuthorHoverCard';   // ← add import
...
const ArticleFeed = () => {
    ...
    // Track which article's author card is open
    const [hoveredAuthor, setHoveredAuthor] = useState(null); // { authorId, authorName, articleId }
    const hoverTimeout = useRef(null);

    const handleMouseEnterAuthor = (authorId, authorName, articleId) => {
        clearTimeout(hoverTimeout.current);
        hoverTimeout.current = setTimeout(() => {
            setHoveredAuthor({ authorId, authorName, articleId });
        }, 300); // slight delay so fast mouse-overs don't trigger
    };
    ...
    const handleCardMouseEnter = () => {
        clearTimeout(hoverTimeout.current); // keep card open when hovering it
    };
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <div className="space-y-12">
                {articles.map((article) => {
                    ...
                    const authorId = rawAuthor?._id?.toString() || rawAuthor?.toString() || null;
                    const isHovered = hoveredAuthor?.articleId === article._id;
                        ...
                        <article key={article._id} className="group">
                            <div className="flex flex-col md:flex-row gap-6">
                                <div className="flex-1 order-2 md:order-1">
                                    ...
                                    {/* ── Author row — only avatar + name, no duplicate ── */}
                                    <div className="relative flex items-center space-x-2 mb-2 w-fit">
                                        <div
                                            className="flex items-center space-x-2 cursor-pointer"
                                            onMouseEnter={() => handleMouseEnterAuthor(authorId, article.authorName, article._id)}
                                ...
                                >
                                    {article.coverImageUrl ? (
                                        ...
                                        <div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 rounded shadow-sm flex items-center justify-center">
